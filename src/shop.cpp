...
Shop::Shop(Game& game, Player& player)
    : game{game}, player{player}, click(KEY_DOWN_MOUSE(SDL_BUTTON_LMASK)) {
    // Create buttons.
    ...
    const Button EXIT_SHOP_BUTTON{{X + BUTTON_W + BUTTON_SPACING, Y, BUTTON_W, BUTTON_H},
                                  "EXIT SHOP",
                                  [](Player&, Game& g) { g.start_level(); }};
    const Button INCREASE_JUMP_BUTTON{
        {X, Y + BUTTON_H + BUTTON_SPACING, BUTTON_W, BUTTON_H},
        "INCREASE\nJUMP",
        [](Player& p, Game&){
            if (p.take_coins(1)) {
                p.increase_jump();
...
void Shop::draw() const {
    ...
    SDL_FRect bg{
        0, 0, static_cast<f32>(gWindow.get_width()), static_cast<f32>(gWindow.get_height())};
    ...
    if (gAssets.font) {
        ...
        if (title_surface) {
            ...
            if (title_texture) {
                SDL_FRect title_rect{f32(gWindow.get_width() - title_surface->w) / 2,
                                     20,
                                     f32(title_surface->w),
                                     f32(title_surface->h)};
    ...
    // Draw player stats.
    const std::string stats = "Coins: " + std::to_string(player.get_coins()) +
