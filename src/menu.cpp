...
void Menu::render_button(const Button& button, SDL_Renderer* renderer, TTF_Font* font) const {
  ...
  SDL_Color bg_color =
      button.hovered ? SDL_Color{100, 150, 255, 255} : SDL_Color{50, 100, 200, 255};
  ...
  if (font) {
    ...
    if (text_surface) {
      ...
      if (text_texture) {
        SDL_FRect text_rect{button.rect.x + (button.rect.w - text_surface->w) / 2,
                            button.rect.y + (button.rect.h - text_surface->h) / 2,
                            f32(text_surface->w), f32(text_surface->h)};
